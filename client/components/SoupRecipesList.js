...
const SoupRecipesList = ({ navigation, route }) => {
  const [Recipes, setRecipes] = useState();
  const anotherRecipes = async () => {
    try {
      const token = route.params[2];
      const res = await recipesApi.changeSoupRecipes(token);
      if (res.data) {
        setRecipes(res.data);
      }
    } catch (e) {
  ...
  const changeRecipes = async (afterId) => {
    try {
      ...
      Alert.alert("セッションが切れました\n再度ログインしてください");
      navigation.navigate("ログイン");
      console.error("changeSoupRecipe :", e);
    }
  };
...
export default SoupRecipesList;
